...
public final class DisableMobGriefing extends JavaPlugin implements Listener {
    ...
    public void onEntityExplode(EntityExplodeEvent event) {
        ...
        if (!config.getBoolean(type.name().toLowerCase() + "_griefing", true)) {

           // Prevent block damage ONLY
         event.blockList().clear();

         if (isVerbose) {
             getLogger().info(type + " explosion block damage disabled (entity damage allowed).");
         }
    }
}
